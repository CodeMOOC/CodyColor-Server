...
broker.connect({
  ...
  onSignUpRequest: function (message) {
    ...
    if (message.wallUser) {
      let insertUser =
        "INSERT INTO WallUsers (Id, Name, Surname, Deleted) " +
        "VALUES (" +
        database.escape(message.userId) +
        ", " +
  ...
  onLogInRequest: function (message) {
    ...
    if (message.wallUser) {
      ...
      let bestMatchHuman =
        ...
        "                  WHERE MP2.UserId = " +
        database.escape(message.userId) +
        " AND MP2.IsWallUser = 1) " +
        "AND MP1.IsWallUser = 0 " +
        "ORDER BY points DESC, pathLength DESC, time DESC " +
        "LIMIT 1; ";

      let queries =
        searchUser + userTotalMatches + bestMatchBot + bestMatchHuman;
      ...
      database.query(queries, function (results, error) {
        ...
        if (error) {
          ...
          if (results[2] !== undefined && results[2][0] !== undefined) {
            bestMatchBotValue = results[2][0];
          }

          let bestMatchHumanValue = undefined;
          if (results[3] !== undefined && results[3][0] !== undefined) {
      ...
      let queries =
        searchUser + userTotalPoints + userWins + userAvgPoints + bestMatch;
      ...
      database.query(queries, function (results, error) {
        ...
        if (error) {
          ...
          if (results[4] !== undefined && results[4][0] !== undefined) {
  ...
  onUserDeleteRequest: function (message) {
    ...
    if (message.wallUser) {
      let deleteUser =
        "UPDATE WallUsers SET Name = '', Surname = '', Deleted = 1 " +
        "WHERE Id = " +
        database.escape(message.userId);
      ...
      let deleteUser =
        "UPDATE Users SET Email = '', Deleted = 1 " +
        "WHERE Id = " +
        database.escape(message.userId);
  ...
  onRankingRequest: function (message) {
    ...
    let queries =
      top10PointsDaily + top10PointsGlobal + top10MatchDaily + top10MatchGlobal;
    database.query(queries, function (results, error) {
      let response = {};
      if (error) {
        response = {
  ...
  onPlayerQuit: function (message) {
    ...
    // giocatori rimasti
    logs.printLog(
      "Received playerQuit request from " +
        message.gameType +
        " client " +
        +message.gameRoomId +
        "[" +
    ...
    if (result.success) {
      logs.printLog(
        "User removed from " + message.gameType + " game rooms array"
      );
      gameRoomHandler.printGameRooms();
    } else {
      logs.printLog(
        "WARNING: The user is not present in the game room " +
          "[" +
          message.gameRoomId +
          "]"
      );
  ...
  onHeartbeat: function (message) {
    ...
    if (!result.success) {
      logs.printLog(
        "Received invalid heartbeat. Trying to disconnect the user"
      );
      logs.printWaiting();
    }
  },
  ...
  onPositioned: function (message) {
    ...
    logs.printLog(
      "Received positioned message from " +
        message.gameType +
        " client " +
        +message.gameRoomId +
        "[" +
  ...
  onEndAnimation: function (message) {
    ...
    logs.printLog(
      "Received endAnimation message from " +
        message.gameType +
        " client " +
        +message.gameRoomId +
        "[" +
...
gameRoomCallbacks = {
  onGameRoomsUpdated: function () {
    ...
    if (gInfoLimiter) {
      ...
      setTimeout(function () {
        if (gInfoQueued) sendGeneralInfoMessage();
        gInfoLimiter = false;
        gInfoQueued = false;
      }, 1000);
    }
  ...
  createDbGameSession: function (gameRoomData) {
    ...
    let insertSession =
      "INSERT INTO GameSessions (NumMatches, Type, MatchDurationMs, BeginTimestamp) " +
      "VALUES (0, " +
  ...
  createDbGameMatch: function (gameRoomData) {
    ...
    let updateSession =
      "UPDATE GameSessions SET NumMatches = " +
      gameRoomData.gameData.matchCount +
      " " +
      "WHERE Id = " +
    ...
    let insertMatch =
      "INSERT INTO GameMatches (SessionId, BeginTimestamp, NumUsers) " +
      "VALUES (" +
      gameRoomData.sessionId +
      ", " +
      ...
      if (!error) {
        ...
        for (let i = 0; i < gameRoomData.players.length; i++) {
          if (gameRoomData.players[i].occupiedSlot) {
            ...
            let nickname;
            if (gameRoomData.players[i].userId !== undefined) {
              nickname = database.escape(
            ...
            let winner =
              gameRoomData.players[i].gameData.match.winner === true ? 1 : 0;
            let registered =
              gameRoomData.players[i].userId !== undefined ? 1 : 0;
            let isWallUser =
              gameRoomData.players[i].gameData.organizer && gameRoomData.isWall
                ? 1
                : 0;
...
let sendGeneralInfoMessage = function (correlationId) {
  ...
  database.query(totalMatches, function (results, error) {
    ...
    let connectedPlayers =
      randomGameRooms.getConnectedPlayers() +
      customGameRooms.getConnectedPlayers() +
    ...
    if (message.correlationId === undefined) broker.sendInGeneralTopic(message);
    else broker.sendInClientControlQueue(message.correlationId, message);
